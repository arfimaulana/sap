<?php 
	...
	$koneksi = mysqli_connect("localhost","root","","regio_personnel");
	...
	if (mysqli_connect_errno()){
		...
		// 			nik AS `f01`,
		// 			DATE_FORMAT(MAX(scan_time),'%Y') AS `f02`,
		// 			DATE_FORMAT(MAX(scan_time),'%m') AS `f03`,
		// 			DATE_FORMAT(MAX(scan_time),'%d') AS `f04`,
		// 			DATE_FORMAT(MAX(scan_time),'%H') AS `f05`,
		// 			DATE_FORMAT(MAX(scan_time),'%i') AS `f06`,
		// 			NULL AS `f07`,
		// 			NULL AS `f08`,
		// 			NULL AS `f09`,
		...
		// 			WHERE scan_time BETWEEN '".$new." 10:00:00' AND '".$new." 23:59:59' AND io = '0' AND sys_users.`role_id` = '6'
		// 			GROUP BY user_id ORDER BY sys_data_scan.`id` ASC";
		$mysql = "SELECT u.nama AS ID, 
					u.nik AS f01, 
					DATE_FORMAT(MAX(a.scan_date), '%Y') AS f02, 
					DATE_FORMAT(MAX(a.scan_date), '%m') AS f03,
					DATE_FORMAT(MAX(a.scan_date), '%d') AS f04,
					DATE_FORMAT(MAX(a.scan_date), '%H') AS f05,
					DATE_FORMAT(MAX(a.scan_date), '%i') AS f06,
					NULL AS f07,
					NULL AS f08,
					NULL AS f09,
					...
					WHERE a.scan_date BETWEEN '".$new." 10:00:00' AND '".$new." 23:59:59' AND g.io = '0' AND u.role_id = '6'
					GROUP BY a.pin";
