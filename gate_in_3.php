...
function pingAddress($ip)
{
    ...
    if (0 == $status) {
        ...
        $mysql = "UPDATE gate_status SET gate_status = 'Online', last_pinged = NOW() WHERE id = '3'";
        $koneksi->query($mysql);
    } else {
        // $status = "dead";
        $mysql = "UPDATE gate_status SET gate_status = 'Offline', last_pinged = NOW() WHERE id = '3'";
        $koneksi->query($mysql);
    }
    echo "<br>";
